 ...
 * Time: 14:31  .
 */
public abstract class AbstractCheckStringValidator implements ConstraintValidator<CheckString, String> {
    ...
    private final boolean validatorMaxLength(String field, ConstraintValidatorContext constraintValidatorContext) {
        ...
        if (minLength != -1 && field.length() > maxLength) {
            ...
            constraintValidatorContext.buildConstraintViolationWithTemplate(Constants.CONSTRAINT_STRING_PARAM_MAXLENGTH_ERROR + maxLength).addConstraintViolation();
            result = false;
        }
        return result;
    }

    public abstract boolean validatorAdditional(String field, ConstraintValidatorContext constraintValidatorContext);
